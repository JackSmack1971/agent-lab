...
class UseCaseInput(BaseModel):
    ...
    description: str = Field(..., description="Natural language description of the use case")
    max_cost: Optional[float] = Field(None, description="Maximum acceptable cost per 1K tokens")
    min_speed: Optional[float] = Field(None, description="Minimum acceptable speed (tokens/second)")
    context_length_required: Optional[int] = Field(None, description="Minimum context window required")
...
class SuggestedConfig(BaseModel):
    ...
    temperature: float = Field(..., ge=0.0, le=2.0, description="Suggested temperature setting")
...
class ModelRecommendation(BaseModel):
    ...
    suggested_config: SuggestedConfig = Field(..., description="Suggested configuration parameters")
    estimated_cost_per_1k: float = Field(..., ge=0.0, description="Estimated cost per 1K tokens")
...
class RecommendationResponse(BaseModel):
    ...
    recommendations: list[ModelRecommendation] = Field(..., description="Top 3 model recommendations")
    analysis_summary: str = Field(..., description="Overall analysis summary of the recommendations")
