...
class TestKeyboardShortcut:
    ...
    def test_get_normalized_combination_without_overrides(self):
        ...
        with patch('src.utils.keyboard_handler.PlatformDetector.normalize_combination') as mock_normalize:
...
class TestPlatformDetector:
    ...
    @pytest.mark.parametrize('combination,platform,expected', [
...
class TestContextManager:
    ...
    @pytest.mark.parametrize('shortcut_context,context_state,expected', [
    ...
    def test_is_shortcut_available(self, context_manager, shortcut_context, context_state, expected):
...
class TestKeyboardHandler:
    ...
    def test_register_shortcut_with_conflicts(self, keyboard_handler, mock_platform_detector):
        ...
        with patch.object(keyboard_handler, 'check_conflicts', return_value=['conflict1']):
            with patch('src.utils.keyboard_handler.logger') as mock_logger:
                keyboard_handler.register_shortcut(shortcut1)
                keyboard_handler.register_shortcut(shortcut2)
    ...
    def test_process_event_success(self, keyboard_handler, mock_platform_detector, mock_context_manager):
    ...
    def test_process_event_rate_limited(self, keyboard_handler):
        ...
        keyboard_handler._event_history = [current_time - 0.05] * (MAX_EVENTS_PER_SECOND + 1)
    ...
    def test_process_event_disabled_shortcut(self, keyboard_handler, mock_platform_detector):
    ...
    def test_process_event_not_available(self, keyboard_handler, mock_platform_detector, mock_context_manager):
    ...
    def test_get_available_shortcuts(self, keyboard_handler, mock_context_manager):
        ...
        mock_context_manager.is_shortcut_available.side_effect = lambda s, c: s.id == 'available'
    ...
    def test_register_shortcut_general_exception_handling(self, keyboard_handler):
        ...
        with patch.object(keyboard_handler, 'check_conflicts', side_effect=Exception('Conflict check error')):
            with patch('src.utils.keyboard_handler.logger') as mock_logger:
                with pytest.raises(Exception, match='Conflict check error'):
                    keyboard_handler.register_shortcut(shortcut)
                mock_logger.error.assert_called_once_with('Shortcut registration failed: Conflict check error')

    def test_process_event_general_exception_handling(self, keyboard_handler, mock_platform_detector, mock_context_manager):
        ...
        with patch.object(keyboard_handler, '_check_rate_limit', side_effect=Exception('Rate limit error')):
            with patch('src.utils.keyboard_handler.logger') as mock_logger:
                result = keyboard_handler.process_event(event)
                assert result is None
                mock_logger.error.assert_called_once_with('Event processing failed: Rate limit error')
...
class TestIntegration:
    ...
    def test_end_to_end_shortcut_registration_and_processing(self, real_keyboard_handler):
